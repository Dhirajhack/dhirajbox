...
export const DetailPage: React.FC<DetailPageProps> = ({ box, onBuyNow, onGoBack }) => {
  return (
    <div className="min-h-screen text-white animate-fade-in-up" style={{ animation: 'fade-in-up 0.5s ease-out' }}>
      <div className="container mx-auto px-6 py-12">
        <button onClick={onGoBack} className="flex items-center space-x-2 text-gray-300 hover:text-yellow-400 transition-colors mb-8">
            <ChevronLeftIcon />
            <span>Back to all boxes</span>
        </button>
        <div className="bg-[#18283a] rounded-2xl shadow-2xl shadow-black/40 overflow-hidden md:flex">
          ...
          <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-yellow-400 mt-4">₹{box.price.toLocaleString('en-IN')}</p>
              <p className="text-gray-300 mt-6">{box.detailedDescription}</p>
              
              <div className="mt-8">
                <h3 className="text-xl font-semibold text-white mb-3">Possible Items Inside:</h3>
        ...
        <div className="mt-16">
          ...
          <div className="mt-8 space-y-6 max-w-3xl mx-auto">
            {box.reviews.map(review => (
              <div key={review.id} className="bg-[#18283a] p-6 rounded-xl shadow-lg">
                <div className="flex justify-between items-center">
                  <h4 className="font-bold text-white text-lg">{review.author}</h4>
                  <div className="flex items-center">
                    {[...Array(5)].map((_, i) => (
                      <StarIcon key={i} className={i < review.rating ? 'text-yellow-400' : 'text-gray-600'} />
                    ))}
                  </div>
                </div>
